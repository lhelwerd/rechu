...
class ProductMatcher(Matcher[ProductItem, Product]):
    ...
    def add_map(self, candidate: Product) -> bool:
        ...
        keys = (
            ...
            (self.MAP_GTIN, candidate.shop, candidate.gtin)
        )
        add = False
        for key in keys:
    ...
    def check_map(self, candidate: Product) -> Optional[Product]:
        ...
        if (self.MAP_GTIN, candidate.shop, candidate.gtin) in self._map:
            return self._map[(self.MAP_GTIN, candidate.shop, candidate.gtin)]
        return None
