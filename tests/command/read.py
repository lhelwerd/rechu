...
EXTRA = {
    ...
    "products": [
        ...
        },
        {
            "gtin": 1234567890123,
            "portions": 21,
            "prices": {"minimum": 0.89, "maximum": 1.09},
        },
        {"sku": "abc123", "prices": [8.00], "brand": "A Big Bar of Chocolate"},
        # New product separate from the one before
        {"bonuses": ["disco"], "type": "caramel"},
    ],
}
...
class ReadTest(DatabaseTestCase):
    ...
    def test_run(self) -> None:
        ...
        with self.database as session:
            ...
            self.assertEqual(
                len(products), self.product_count + self.range_count
            )
        ...
        with self.database as session:
            self.assertEqual(
                len(session.scalars(select(Product)).all()),
        ...
        with self.database as session:
            ...
            self.assertEqual(
                products[self.product_count - 1].prices[0].value, Price("8.00")
            )
            self.assertEqual(
                products[self.product_count - 1].brand, "A Big Bar of Chocolate"
            )
            ...
            self.assertEqual(
                [product.price for product in receipt.products],
                [
                    Price("1.99"),
                    Price("5.00"),
                    Price("7.50"),
