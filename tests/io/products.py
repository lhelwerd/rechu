...
class ProductsReaderTest(unittest.TestCase):
    ...
    def _check_product_range(
        self, product: Product, expected: _ProductData
    ) -> None:
...
class ProductsWriterTest(unittest.TestCase):
    ...
    def _check_product(
        self, actual_product: _ProductData, product: _ProductData
    ) -> None:
        self.assertNotIn("shop", actual_product)

        self.assertEqual(
            actual_product.get("labels", []), product.get("labels", [])
        )
        ...
        self.assertEqual(
            actual_product.get("bonuses", []), product.get("bonuses", [])
        )

        self.assertEqual(
            actual_product.get("category"), product.get("category")
        )
        self.assertEqual(actual_product.get("type"), product.get("type"))
        self.assertEqual(
            actual_product.get("description"), product.get("description")
        )
        self.assertEqual(
            actual_product.get("portions"), product.get("portions")
        )
        if "weight" not in actual_product:
            self.assertNotIn("weight", product)
        else:
            self.assertEqual(
                Quantity(actual_product["weight"]), product.get("weight")
            )
        self.assertEqual(actual_product.get("sku"), product.get("sku"))
        if "gtin" not in actual_product:
            self.assertNotIn("gtin", product)
        else:
            self.assertEqual(GTIN(actual_product["gtin"]), product.get("gtin"))
    ...
    def test_serialize_invalid(self) -> None:
        ...
        with self.assertRaisesRegex(
            ValueError, "Not all products are from the same shop"
        ):
            writer.serialize(file)

        writer = ProductsWriter(
            self.path, (*self.models, self.models[-1].range[-1])
        )

        with self.assertRaisesRegex(
            ValueError, "Product .* is not generic but range"
        ):
            writer.serialize(file)
        ...
        with self.assertRaisesRegex(
            ValueError, "Not all price matchers have indicators"
        ):
            writer.serialize(file)
