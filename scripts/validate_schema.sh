...
check() {
    ...
    if [[ "$schema" = "schema/receipt.json" ]]; then
        ...
        output=$(check-jsonschema $args $files 2>&1 |
            grep -v "\d.\d\d\? is not a multiple of 0.01" |
            sed -e "$ s/^Schema validation errors were encountered.$//")
        if [ "$output" = "" ]; then
...
check schema/settings.json --default-filetype toml \
    "$ROOT_DIR/rechu/settings.toml" "$ROOT_DIR/tests/settings.toml"
...
check schema/pyproject.json "$ROOT_DIR/pyproject.toml" \
    "$ROOT_DIR/tests/settings.prefix.toml"
